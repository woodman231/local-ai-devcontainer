...
async def main():    
    ...
    embedded_query_text = await ollama_client.embed(
        model="mxbai-embed-large",
        input=query_text,
    )
    embedded_query_text = embedded_query_text.embeddings[0]
    ...
    try: 
        response = await ollama_client.chat(
            model="llama3",
            messages=[Message(role="user", content=GROUNDED_PROMPT.format(
                query=query_text,
